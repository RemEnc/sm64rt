...
s32 act_handstand_transition(struct MarioState *m) {
	play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_HOOHOO);

    if (mario_check_object_grab(m)) {
        mario_grab_used_object(m);
        m->marioBodyState->grabPos = GRAB_POS_LIGHT_OBJ;
        if (m->action != ACT_HANDSTAND_TRANSITION) {
            return TRUE;
        }
    }
	
	set_mario_animation(m, MARIO_ANIM_GROUND_HANDSTAND_INTO);
	update_air_without_turn(m);
	
    switch (perform_air_step(m, AIR_STEP_CHECK_HANG)) {
        case AIR_STEP_NONE:
			set_mario_animation(m, MARIO_ANIM_GROUND_HANDSTAND_INTO);
            break;
        ...
        case AIR_STEP_LANDED:
            if (should_get_stuck_in_ground(m)) {
                ...
                m->particleFlags |= PARTICLE_MIST_CIRCLE;
                drop_and_set_mario_action(m, ACT_HEAD_STUCK_IN_GROUND, 0);
            } else if (!check_fall_damage(m, ACT_HARD_BACKWARD_GROUND_KB)) {
                ...
                if (m->heldObj == NULL) {
					if (is_anim_at_end(m)) {
						play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
						set_mario_action(m, ACT_HANDSTAND, 0);
					}
                } else {
                    set_mario_action(m, ACT_DIVE_PICKING_UP, 0);
                }
	...
	//common_air_action_step(m, ACT_HANDSTAND, MARIO_ANIM_START_HANDSTAND, AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG);

    return FALSE;
}

s32 act_handstand_jump(struct MarioState *m) {
	play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_YEEHAW);
    common_air_action_step(m, ACT_DOUBLE_JUMP_LAND, MARIO_ANIM_HANDSTAND_JUMP, AIR_STEP_CHECK_LEDGE_GRAB);
    return FALSE;
}
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    ...
    switch (m->action) {
		...
		case ACT_HANDSTAND_TRANSITION: cancel = act_handstand_transition(m); break;
		case ACT_HANDSTAND_JUMP:	   cancel = act_handstand_jump(m);		 break;
    }
    /* clang-format on */
