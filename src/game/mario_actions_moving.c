...
s32 act_handstand_walking(struct MarioState *m) {
    s32 val04;
    ...
    if (m->input & INPUT_FIRST_PERSON) {
        return set_mario_action(m, ACT_HANDSTAND_TO_CROUCH, 0);
    }

    if (m->input & INPUT_A_PRESSED) {
        return set_jumping_action(m, ACT_HANDSTAND_JUMP, 0);
    }

    if (m->input & INPUT_UNKNOWN_5) {
        return set_mario_action(m, ACT_HANDSTAND, 0);
    }
    ...
    val04 = (s32)(m->intendedMag * 0.06f * 0x10000);
    set_mario_anim_with_accel(m, MARIO_ANIM_GROUND_HANDSTAND_WALKING, val04);
    play_step_sound(m, 12, 29);
    return FALSE;
}
...
s32 mario_execute_moving_action(struct MarioState *m) {
    ...
    switch (m->action) {
		...
		case ACT_HANDSTAND_WALKING:		   cancel = act_handstand_walking(m);		 break;
    }
    /* clang-format on */
